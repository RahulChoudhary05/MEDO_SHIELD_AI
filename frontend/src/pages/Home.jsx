...
const FLOW_STEPS = [
  {
    ...
    items: ['Patient & Doctor register/login', 'JWT role-based access', 'Dashboard access granted'],
  },
  {
    step: '02',
    ...
    items: ['Upload PDF medical reports', 'Add symptom queries & history', 'Secure cloud + DB storage'],
  },
  {
    step: '03',
    ...
    items: ['OCR + medical entity extraction', 'Abnormality & risk detection', 'Patient-friendly plain-language summary'],
  },
  {
    step: '04',
    ...
    items: ['Symptoms + labs + age analysis', 'Specialization suggestion', 'Urgency priority scoring'],
  },
  {
    step: '05',
    ...
    items: ['View AI insights & extractions', 'Add clinical opinion', 'Upload prescription & guidance'],
  },
  {
    step: '06',
    ...
    items: ['All reports + AI summaries', 'Trend graphs & comparisons', 'Doctor feedback history'],
  },
];
  ...
  return (
    <div className="min-h-screen bg-dark text-white">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-secondary/10 border border-secondary/30 mb-6">
                ...
                <span className="text-secondary text-xs font-semibold tracking-widest uppercase">MEDO SHIELD AI Platform</span>
              </div>
              ...
              <p className="text-slate-300 mt-6 text-lg leading-relaxed max-w-xl">
                Upload your medical reports, get AI-powered plain-language summaries,
                connect with doctors for expert review, and track your complete health journey — all in one platform.
              </p>

              <div className="flex flex-wrap gap-3 mt-8">
                {token ? (
                  <Link to="/dashboard" className="btn-teal text-base px-8 py-3.5">
                    Go to Dashboard
                  ...
                  <>
                    ...
                    <Link to="/login" className="btn-outline border-white/20 text-white text-base hover:border-secondary hover:text-secondary">
                      Patient Login
                    </Link>
                    <Link to="/doctor-login" className="btn-outline border-white/20 text-white text-base hover:border-secondary hover:text-secondary">
                      Doctor Login
                    </Link>
                  </>
                )}
              ...
              <div className="mt-10 grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={s.label} className="bg-white/5 border border-white/10 rounded-xl p-3">
                    ...
                    <p className="text-sm font-semibold mt-1 text-white">{s.value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-sm">
              <div className="flex items-center justify-between mb-6">
                <p className="text-sm font-semibold text-slate-300 uppercase tracking-wider">AI Report Pipeline</p>
              ...
              <div className="space-y-3">
                {[
                  { label: 'PDF Upload', status: 'Ready', color: 'text-emerald-400' },
                  { label: 'OCR Extraction', status: 'AI Processing', color: 'text-secondary' },
                  { label: 'Risk Detection', status: 'Gemini AI', color: 'text-secondary' },
                  { label: 'Plain Summary', status: 'Generated', color: 'text-emerald-400' },
                  { label: 'Doctor Review', status: 'Async Queue', color: 'text-amber-400' },
                  { label: 'Health Timeline', status: 'Updated', color: 'text-emerald-400' },
                ].map((row, i) => (
              ...
              <div className="mt-6 p-4 bg-secondary/10 border border-secondary/30 rounded-xl">
                <p className="text-xs text-secondary font-semibold uppercase tracking-wider mb-1">Latest AI Insight</p>
                <p className="text-sm text-slate-300 italic">
                  "Your hemoglobin is slightly low, which may indicate mild anemia. Iron-rich diet and a general physician visit is recommended."
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="bg-[#0f172a] py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-secondary text-xs uppercase tracking-widest font-semibold mb-3">Key Features</p>
            <h2 className="text-4xl font-bold">Everything in one portal</h2>
            <p className="text-slate-400 mt-4 max-w-2xl mx-auto">
              From report upload to expert doctor review — MEDO SHIELD AI handles every step of your health journey.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map(f => (
              <div key={f.title} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/8 hover:border-secondary/30 transition-all duration-300 group">
      ...
      <section className="py-20 px-6 bg-dark">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-secondary text-xs uppercase tracking-widest font-semibold mb-3">System Workflow</p>
            <h2 className="text-4xl font-bold">Step-by-step care journey</h2>
            <p className="text-slate-400 mt-4 max-w-2xl mx-auto">
              A clean, structured workflow that keeps patients informed and doctors in full control.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {FLOW_STEPS.map(s => (
              <div key={s.step} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-secondary/30 transition-all duration-300">
                <div className="flex items-center gap-4 mb-4">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center flex-shrink-0`}>
                    {s.icon}
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 font-semibold">Step {s.step}</p>
      ...
      <section className="py-20 px-6 bg-[#0f172a]">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
            ...
            <p className="text-slate-400 mb-6">Everything you need to manage your health digitally.</p>
            <div className="grid sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={item.t} className="bg-dark/60 border border-white/10 rounded-xl p-4">
            ...
            <div className="mt-6 flex gap-3">
              <Link to="/register" className="btn-teal text-sm">Create Account</Link>
              <Link to="/login"    className="btn-outline border-white/20 text-white text-sm hover:border-secondary hover:text-secondary">Patient Login</Link>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-secondary/10 to-primary/20 border border-secondary/20 rounded-3xl p-8">
            ...
            <p className="text-slate-300 mb-6">Asynchronous review model — no live calls needed.</p>
            ...
            <Link to="/doctor-login" className="mt-8 btn-teal text-sm w-full justify-center">
              Enter Doctor Portal
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>
            </Link>
          </div>
      ...
      <section className="py-20 px-6 bg-dark text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-bold">Ready for smarter health care?</h2>
          <p className="text-slate-400 mt-4 text-lg">All modules are connected, clean, and easy to use.</p>
          <div className="flex flex-wrap gap-4 justify-center mt-8">
            <Link to="/register"     className="btn-teal text-base px-8 py-3.5">Create Patient Account</Link>
            <Link to="/doctor-login" className="btn-outline border-white/20 text-white text-base hover:border-secondary hover:text-secondary">Doctor Portal</Link>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-primary/60 border-t border-white/10 py-8 px-6 text-center">
        <p className="text-slate-400 text-sm">
          &copy; 2026 <span className="text-secondary font-semibold">MEDO SHIELD AI</span> · Privacy-First · Gemini AI Powered · Secure
        </p>
      </footer>
    </div>
  );
